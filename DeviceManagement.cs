...
namespace SMDProfiler
{
	sealed internal partial class DeviceManagement
	{
		...
		internal Boolean getComPortName(String id, ref String name)
		{
			IntPtr deviceInfoSet = new System.IntPtr();
			SP_DEVICE_INTERFACE_DATA MyDeviceInterfaceData = new SP_DEVICE_INTERFACE_DATA();
			SP_DEVINFO_DATA MyDevInfoData = new SP_DEVINFO_DATA();
			Guid comGuid = new Guid(0x4D36E978, 0xE325, 0x11CE, 0xBF, 0xC1, 0x08, 0x00, 0x2B, 0xE1, 0x03, 0x18);
			IntPtr classPtr;

			try
			{
				id = id.Remove(0, 4).Replace('#', '\\');
				id = id.Substring(0, id.LastIndexOf('\\'));

				classPtr = Marshal.StringToHGlobalUni(id);

				deviceInfoSet = DeviceManagement.SetupDiGetClassDevs(ref comGuid, classPtr, IntPtr.Zero, DIGCF_DEVICEINTERFACE);

				MyDeviceInterfaceData.cbSize = Marshal.SizeOf(MyDeviceInterfaceData);
				MyDevInfoData.cbSize = Marshal.SizeOf(MyDevInfoData);

				Int32 RequiredSize;
				Int32 RegType;
				byte[] pBuf = new byte[256];

				if (SetupDiEnumDeviceInfo(deviceInfoSet, 0, ref MyDevInfoData))
				{
					if (SetupDiGetDeviceRegistryProperty(deviceInfoSet, ref MyDevInfoData, SPDRP_FRIENDLYNAME, out RegType, pBuf, 256, out RequiredSize))
					{
						System.Text.ASCIIEncoding encoding = new System.Text.ASCIIEncoding();
						name = encoding.GetString(pBuf);
						String[] sTmp = encoding.GetString(pBuf).Split(new char[] { '(', ')' });
						name = sTmp[1];
		...
		internal int FindDeviceFromGuid(System.Guid myGuid, ref String[] devicePathName)
		{
			Int32 bufferSize = 0;
